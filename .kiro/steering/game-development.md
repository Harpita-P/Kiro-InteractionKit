...
**Note**: The folder is called `my_apps/` (not `my_games/`) because Kiro Motion Kit can be used for any gesture-controlled application, not just games.
...
```python
# Check kiro_motion_kit/controllers/hand_controller.py
...
3. **Create Gesture File** - Add to appropriate folder with proper naming:
   - Hand: `kiro_motion_kit/gestures/hand_gestures/HA#_gesture_name.py`
   - Head: `kiro_motion_kit/gestures/head_gestures/HE#_gesture_name.py`
   - Face: `kiro_motion_kit/gestures/face_gestures/FA#_gesture_name.py`
