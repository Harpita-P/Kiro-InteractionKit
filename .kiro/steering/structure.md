...
## Core Package: `kiro_interaction_kit/`
...
User-created games using the Kiro InteractionKit framework. Each game should be in its own subdirectory.
...
**Important:** All game-specific code (assets, mappings, utilities) must be created within the game's folder under `my_apps/`. The core `kiro_interaction_kit/` library should remain game-agnostic.
